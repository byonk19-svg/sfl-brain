...
import { opportunityScopeForBacklogView } from "@/lib/backlog-view";
...
function Backlog({ rows, held }: { rows: Record<string, unknown>[]; held: boolean }) {
  ...
  return <div className="backlog-list" role="list">{rows.map((row) => { const id = String(row.id); const assets = Number(row.assets_count); const links = Number(row.active_links_count); const products = Number(row.products_count); const itemHeld = Boolean(row.hold_id); const currentHold = itemHeld ? { id: String(row.hold_id), hold_reason: String(row.hold_reason), release_condition: String(row.release_condition), review_on: row.review_on ? String(row.review_on) : null, updated_at: String(row.hold_updated_at) } : undefined; return <article className="backlog-item" key={id} role="listitem"><Link className="backlog-row" href={`/opportunities/${id}`}><div className="backlog-title"><span className="stage-dot" data-stage={String(row.status)} /><div><h2>{String(row.title)}</h2><p>{itemHeld ? "On hold" : humanize(String(row.content_type))}</p></div></div><div className="library-fact"><small>Stage</small><span>{humanize(String(row.status))}</span></div><div className="library-fact"><small>{itemHeld ? "Hold reason" : "Next step"}</small><span>{itemHeld ? String(row.hold_reason) : row.next_action ? String(row.next_action) : "—"}</span></div><div className="library-fact"><small>{itemHeld ? "Release when" : "Effort"}</small><span>{itemHeld ? String(row.release_condition) : `${String(row.estimated_effort_minutes)} min`}</span></div><div className="library-fact"><small>{itemHeld ? "Review" : "Ready pieces"}</small><span>{itemHeld ? row.review_due ? "Due for review" : row.review_on ? formatDate(String(row.review_on)) : "No review date" : `${assets} asset${assets === 1 ? "" : "s"} · ${links} link${links === 1 ? "" : "s"}`}</span></div><div className="library-status">{row.last_published_at ? `Posted ${formatDate(String(row.last_published_at))}` : `${products} product${products === 1 ? "" : "s"}`}</div></Link><OpportunityHoldForm opportunityId={id} currentHold={currentHold} compact /></article>; })}</div>;
}
