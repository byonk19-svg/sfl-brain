...
function PackagePublicationForm({ opportunity, activePackage, item, caption }: {
  ...
  return (
    <form action={recordPostAction} className="editor-form">
      ...
      <section className="form-section"><div className="section-number">03</div><div className="form-section-body"><h2>Ordered assets</h2>{activePackage.assets.length ? <ol>{activePackage.assets.map((selection) => <li key={selection.asset_id}>{selection.asset.title ?? humanize(selection.asset.asset_type)} · {humanize(selection.role)}</li>)}</ol> : <p className="muted">No assets are selected for this package.</p>}<div className="field-grid"><label>Published at<input type="datetime-local" name="published_at" required defaultValue={localDateTime()} /></label><label className="span-2">Publication notes<textarea rows={2} name="notes" /></label></div></div></section>
