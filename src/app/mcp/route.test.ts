...
type McpRouteApi = {
  handleMcpRequest: (request: Request, dependencies: {
    isHosted: () => boolean;
    localHandler: Handler;
    requireAuth: (request: Request) => Promise<AuthInfo | Response>;
    resolveConnectorMember: (authInfo: AuthInfo) => Promise<HostedMember | Response>;
    createHandler: (member: HostedMember) => Handler;
  }) => Promise<Response>;
};

describe("MCP HTTP hosting boundary", () => {
  const api = route as unknown as Partial<McpRouteApi>;
  const request = new Request("https://brain.example.com/mcp", { method: "POST" });

  it("challenges an unauthenticated hosted request", async () => {
    expect(api.handleMcpRequest).toBeTypeOf("function");
    ...
    const response = await api.handleMcpRequest!(request, {
      isHosted: () => true,
  ...
  it("passes verified actor and workspace context to a request-scoped handler", async () => {
    ...
    const createHandler = vi.fn(() => ({ fetch }));
