        ...
        <div className="detail-main">
          <section className="detail-section">
            ...
            {productJoins.length ? <div className="stack-list">{productJoins.map((join) => {
              ...
              return <article className="stack-row opportunity-stack-row" key={text(product.id)}><div><strong><Link href={`/products/${text(product.id)}`}>{text(product.name)}</Link></strong><span>{humanize(text(join.role))}</span></div><div><span>{primary ? `${text(primary.retailer)} · ${humanize(text(primary.stock_status))}` : "No listing"}</span><strong>{primary ? formatMoney(primary.current_price === null ? null : Number(primary.current_price), text(primary.currency) || "USD") : "Product facts pending"}</strong></div><div className="chip-row">{links.filter((link) => link.is_active).map((link) => <span className="chip" key={text(link.id)}>{text(link.network)} ready</span>)}</div>{text(primary?.canonical_url) && <a className="text-button" href={text(primary?.canonical_url)} target="_blank" rel="noreferrer">Open listing</a>}{links.filter((link) => link.is_active).map((link) => <CopyButton key={`copy-${text(link.id)}`} value={text(link.url)} label="Copy affiliate link" />)}<form action={detachOpportunityProductAction}><input type="hidden" name="opportunity_id" value={id} /><input type="hidden" name="product_id" value={text(product.id)} /><button className="text-button" type="submit">Remove</button></form></article>;
          ...
          <section className="detail-section">
            ...
            {assetJoins.length ? <div className="asset-grid">{assetJoins.map((join) => { const asset = record(join.assets); const url = text(asset.signed_url); return <article className="asset-item" key={text(asset.id)}>{url ? <img style={{ width: "100%", aspectRatio: "1", objectFit: "cover" }} src={url} alt={text(asset.title) || `${humanize(text(asset.asset_type))} asset`} /> : <div className="asset-placeholder">File preview unavailable</div>}<strong>{text(asset.title) || "Untitled asset"}</strong><span>{humanize(text(join.role))}</span>{url ? <a className="text-button" href={url} target="_blank" rel="noreferrer">Open stored file</a> : <span className="muted">Original file is unavailable.</span>}<form action={detachOpportunityAssetAction}><input type="hidden" name="opportunity_id" value={id} /><input type="hidden" name="asset_id" value={text(asset.id)} /><button className="text-button" type="submit">Remove</button></form></article>; })}</div> : <p className="muted">No prepared assets are attached yet.</p>}
          ...
          <section className="detail-section">
            ...
            {posts.length ? <div className="timeline">{posts.map((post) => { const destination = record(post.destinations); const caption = text(post.caption); return <article key={text(post.id)}><time>{formatDateTime(text(post.published_at))}</time><div><strong>{text(destination.name) || "Unknown destination"}</strong>{caption ? <><p style={{ whiteSpace: "pre-wrap" }}>{caption}</p><CopyButton value={caption} label="Copy caption" /></> : <p>No caption recorded.</p>}{text(post.angle) && <p className="muted">Angle: {text(post.angle)}</p>}<span className="chip">{humanize(text(post.performance_label))}</span></div></article>; })}</div> : <p className="muted">Never published. When this goes to several destinations, they will appear here under one content idea.</p>}
          </section>
        </div>
