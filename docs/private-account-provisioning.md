...
For local development, keep global `auth.enable_signup = false` while leaving `auth.email.enable_signup = true`. The global setting blocks public registration; the provider setting must remain enabled so owner-provisioned members can sign in with email and password.
